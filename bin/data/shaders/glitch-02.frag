...
void main()
{
    ...
    float glitch = pow(cos(time*0.5)*0.6+1.6, 1.2);
    ...
    nh *= glitch + rnd;
    float r = texture(tex, res * (uv+vec2(nh, 0.08)*nh)).r;
    float g = texture(tex, res * (uv+vec2(nh-0.07, 0.0)*nh)).g;
    float b = texture(tex, res * (uv+vec2(nh, 0.0)*nh)).b;
